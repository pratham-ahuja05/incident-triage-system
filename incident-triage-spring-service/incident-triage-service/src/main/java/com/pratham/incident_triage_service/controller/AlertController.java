...
public class AlertController {
    ...
    public ResponseEntity<Alert> ingestAlert(@Valid @RequestBody AlertRequest request) {
        // Step 1: persist the alert immediately as PENDING
        Alert alert = new Alert();
        alert.setSource(request.getSource());
        alert.setMessage(request.getMessage());
        Alert saved = alertRepository.save(alert);

        // Step 2: push to Redis queue for async processing — controller returns immediately, doesn't wait for AI processing
